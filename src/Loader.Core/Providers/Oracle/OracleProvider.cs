...
namespace Loader.Core.Providers.Oracle;

/// <summary>
/// Provider потокового чтения SQL-запросов из Oracle Database.
/// </summary>
public sealed class OracleProvider : IProvider<IDatabaseSource, SqlTableConfig>
{
    public string Kind => "oracle";
    ...
    {
        var connection = new OracleConnection(source.ConnectionString);
        ...
        {
            // Если открыть reader не удалось, соединение уже некому закрыть через CloseConnection.
