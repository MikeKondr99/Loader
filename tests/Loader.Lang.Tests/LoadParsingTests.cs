...
public sealed class LoadParsingTests
{
    ...
    [Test]
    [DisplayName("Script parser допускает LOAD без имени таблицы для semantic validation")]
    public async Task Load_without_table_name_parses_for_semantic_validation()
    {
        var result = Loader.Lang.Script.Parse("LOAD * FROM Inline(x; 1);");

        await Assert.That(result.IsSuccess).IsTrue();
        var load = (LoadStatement)result.Value!.Statements[0];
        await Assert.That(load.TableName).IsNull();
        await Assert.That(load.TableNameSpan).IsNull();
        await Assert.That(load.LoadSpan).IsEqualTo(new LangSpan(1, 0, 1, 4));
    }
    ...
    [Test]
    [Arguments("tmp: LOAD id AS id FROM Csv(path='orders.csv', delimiter=null);")]
    [Arguments("tmp: LOAD id AS id FROM Csv(path='orders.csv' delimiter=',');")]
    [Arguments("tmp: LOAD id AS id FROM Csv(path='orders.csv') (csv delimiter=',' header=true);")]
    [DisplayName("LOAD source options запрещает null и пропущенные запятые")]
    ...
    [Test]
    [Arguments("")]
    [Arguments("tmp: LOAD * FROM Csv(path='orders.csv')")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv')")]
    [Arguments("tmp: LOAD FROM Csv(path='orders.csv');")]
    [Arguments("tmp: LOAD id [orders.csv];")]
    [Arguments("tmp: LOAD id FROM;")]
    [Arguments("tmp: LOAD id FROM orders.csv;")]
    [Arguments("tmp: LOAD * id FROM Csv(path='orders.csv');")]
    [Arguments("tmp: LOAD *, id FROM Csv(path='orders.csv');")]
    ...
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv' delimiter=',');")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv', delimiter=);")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv', delimiter=null);")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv', delimiter=',',); extra")]
    [Arguments("tmp: LOAD id WHERE active FROM Csv(path='orders.csv');")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') WHERE;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') WHERE amount > 10 WHERE active;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') WHERE amount > 10 (csv);")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER BY;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER BY id,, name;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER BY id WHERE active;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER id;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') GROUP;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') GROUP BY;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') GROUP BY id,, name;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') GROUP BY id WHERE active;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') ORDER BY id GROUP BY id;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') OFFSET 10;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10.5;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT -1;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10 OFFSET;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10 OFFSET 2.5;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10 LIMIT 20;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10 WHERE active;")]
    [Arguments("tmp: LOAD id FROM Csv(path='orders.csv') LIMIT 10 ORDER BY id;")]
    [Arguments("tmp: LOAD * FROM Inline(a;);")]
    [Arguments("tmp: LOAD * FROM Inline(; 1);")]
